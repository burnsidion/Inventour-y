...
router.delete("/:id", authenticateToken, async (req, res) => {
	...
	try {
		...
		const tours = await client.query(
			"SELECT id FROM tours WHERE user_id = $1",
			[userId]
		);
		...
		if (tourIds.length > 0) {
			...
			const shows = await client.query(
				"SELECT id FROM shows WHERE tour_id = ANY($1)",
				[tourIds]
			);
			...
			if (showIds.length > 0) {
				...
				await client.query("DELETE FROM sales WHERE show_id = ANY($1)", [
					showIds,
				]);

				// Delete all show summaries
				await client.query(
					"DELETE FROM show_summaries WHERE show_id = ANY($1)",
					[showIds]
				);
			...
			await client.query("DELETE FROM inventory WHERE tour_id = ANY($1)", [
				tourIds,
			]);
